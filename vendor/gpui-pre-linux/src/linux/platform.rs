...
///
/// This snapshot of GPUI has no hidden/`None` variant on [`CursorStyle`]
/// (official `gpui` gained one; `gpui-pre` predates it), so one existing style
/// is repurposed to mean "hide the pointer". Hiding is a *null cursor surface*
/// on Wayland, which neither the cursor-shape protocol nor a named cursor icon
/// can express, so every site that turns a style into a cursor has to check
/// this first.
///
/// `IBeamCursorForVerticalLayout` is the stand-in because it is the variant an
/// application is least likely to want by accident — it is the caret for
/// vertical writing-mode text, and nothing inside GPUI ever sets it.
pub(super) fn is_hidden_cursor_style(style: CursorStyle) -> bool {
    matches!(style, CursorStyle::IBeamCursorForVerticalLayout)
}
...
pub(super) fn cursor_style_to_icon_names(style: CursorStyle) -> &'static [&'static str] {
    ...
    match style {
        ...
        // Local patch: this style means "hidden" (see `is_hidden_cursor_style`),
        // and the Wayland backend intercepts it before ever reaching this
        // lookup. Only X11 gets here, where hiding is not implemented — so it
        // falls back to a plain arrow rather than drawing a vertical-text
        // caret, leaving X11 exactly as it behaved before this patch: the laser
        // dot shows, alongside the normal cursor.
        CursorStyle::IBeamCursorForVerticalLayout => &[DEFAULT_CURSOR_ICON_NAME],
