...
impl WaylandClientState {
    ...
    fn restore_cursor_after_hide(&mut self) {
        ...
        let serial = self.serial_tracker.get(SerialKind::MouseEnter);
        // Local patch: restoring a hidden style means re-hiding, not drawing it.
        if is_hidden_cursor_style(style) {
            if let Some(wl_pointer) = self.wl_pointer.clone() {
                wl_pointer.set_cursor(serial.as_raw(), None, 0, 0);
            }
            return;
        }
...
impl LinuxClient for WaylandClient {
    ...
    fn set_cursor_style(&self, style: CursorStyle) {
        ...
        // Local patch: a null cursor surface, which the branches below cannot express.
        if is_hidden_cursor_style(style) {
            ...
            wl_pointer::Event::Enter {
                ...
                if let Some(window) = get_window(&mut state, &surface.id()) {
                    ...
                    if let Some(style) = state.cursor_style {
                        // Local patch: stay hidden across a re-enter.
                        if is_hidden_cursor_style(style) {
